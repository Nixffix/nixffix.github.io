    ...
    if (window.location.href.includes('product')) {
        ...
        if ( productCarFromArr ) {
            ...
            function loadGalleryImages(baseUrl) {
                const basePath = baseUrl.slice(0, -5); // удаляем '1.jpg' или аналогичное
                ...
                function tryLoadImage(i) {
                    ...
                    testImg.onload = () => {
                        const fullImgUrl = `${basePath}${i}.jpg`;
                    ...
                    testImg.onerror = () => {
                        initGallerySwiper(); // Инициализируем Swiper после загрузки всех картинок
                        ...
                        if ( document.querySelector('.swiper-slide') ) {
                            ...
                            const mainSlide = document.createElement('div');
                            mainSlide.classList.add('swiper-slide');
                            mainSlide.innerHTML = `<img src="../images/logo-none-lil.png" alt="photo">`;
                            document.querySelector('.product__image .swiper-wrapper').appendChild(mainSlide);
            
                            const thumbSlide = document.createElement('div');
                            thumbSlide.classList.add('swiper-slide');
                            thumbSlide.innerHTML = `<img src="../images/logo-none-lil.png" alt="photo">`;
                            document.querySelector('.product__gallery .swiper-wrapper').appendChild(thumbSlide);
                        }

                    };
                    testImg.src = `${basePath}${i}.jpg`;
                }
            ...
            document.querySelectorAll('.calc__price-rub')[2].textContent = `${String(parseInt((productCarFromArr.price * 10000) * valueKRW)).replace(/\B(?=(\d{3})+(?!\d))/g, '.')} ₽`;
            document.querySelectorAll('.calc__price-krw')[2].textContent = `${String(parseInt((productCarFromArr.price * 10000))).replace(/\B(?=(\d{3})+(?!\d))/g, '.')} ₩`;
            document.querySelectorAll('.calc__price-usd')[2].textContent = `${fromKRWtoUSD(String(productCarFromArr.price * 10000))} $`;
